...
export const FacultyRegistrationForm: React.FC = () => {
    ...
    return (
      <div className="p-6 text-center space-y-4 animate-in fade-in duration-200">
        ...
        <div className="space-y-1">
          <h3 className="text-xl font-bold text-slate-900">Faculty Account Registered</h3>
          <p className="text-xs text-slate-600">
            Faculty profile for <span className="font-bold text-blue-600">Prof. {formData.fullName}</span> ({formData.department}) created successfully.
          </p>
          <p className="text-[11px] text-slate-400 font-medium">
            Your mentee mapping will activate upon institutional verification.
          </p>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="block text-xs font-medium text-slate-700">Academic Department</label>
          <select
      ...
      <div className="space-y-1 pt-1">
        <label className="flex items-start space-x-2 text-xs text-slate-600 cursor-pointer">
          ...
          />
          <span>
            I agree to the InternIQ Faculty Mentor guidelines and institutional evaluation policies.
          </span>
        </label>
      ...
      >
        {isLoading ? 'Registering Faculty Account...' : 'Register Faculty Mentor'}
      </Button>
    </form>
  );
};
